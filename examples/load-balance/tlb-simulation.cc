...
int main (int argc, char *argv[])
{
    ...
    sourceApps.Start (Seconds (0.0 + 0.01 * i));
    sourceApps.Stop (Seconds (0.1));
    }


    OnOffHelper sourceC ("ns3::UdpSocketFactory", InetSocketAddress (i4i6.GetAddress(1), port+1));
    ...
    PacketSinkHelper sink2 ("ns3::UdpSocketFactory",
            InetSocketAddress (i4i6.GetAddress(1), port+1));
    ...
    if (transportProt.compare ("DcTcp") == 0)
    {
        ...
        Ptr<QueueDisc> queueDisc2 = qd1d3.Get (0);
    ...
    Ptr<PacketSink> pktSink = sinkApp2.Get (0)->GetObject<PacketSink> ();
