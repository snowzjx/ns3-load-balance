...
def parse (fileName, id):
    ...
    for sim in sim_list:
	    ...
	    total_lost_packets += flow.lostPackets
            ...
            if id == 1:
                file1Map[t] = flow.txPackets
            else:
                file2Map[t] = flow.txPackets
...
def main (argv):
    ...
    for flowKey in file1Map.iterkeys():
        flow1Size = file1Map[flowKey]
        flow2Size = file2Map[flowKey]
        if flow1Size < flow2Size:
            print "Flow (%s/%s --> %s/%s), Size1: %i and Size2: %i" % (flowKey.sourceAddress, flowKey.sourcePort, flowKey.destinationAddress, flowKey.destinationPort, flow1Size, flow2Size) 
