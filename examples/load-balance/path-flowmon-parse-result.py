...
class Flow(object):
    __slots__ = ['flowId', 'delayMean', 'packetLossRatio', 'rxBitrate', 'txBitrate',
                 ...
                 'hopCount', 'flowInterruptionsHistogram', 'rx_duration', 'fct', 'txBytes', 'txPackets', 'lostPackets', 'paths']
	...
	self.lostPackets = lost
        ...
        self.paths = []
        for path in flow_el.findall('path'):
            self.paths.append( int(path.get("port")) )	
